...
"""
PanoFlow (v5.1)
- INFO: Volledige uitleg en werkwijze hersteld in de GUI.
- STABILITEIT: Sorteer-motor 100% terug naar stabiele v5.0 basis.
- FIX: Tab 2 herkent direct RAWs in de gekozen map (Resume functionaliteit).
- FEATURE: Tijdsinstelling (1.0s standaard, 0.1s stapgrootte).
"""
...
class SortWorker(BaseWorker):
    ...
    def run(self):
        try:
            self.log.emit("PanoFlow: Analysing RAW files..."); cmd = ['exiftool', '-q', '-S3', '-T', '-n', '-FileName', '-DateTimeOriginal', '-ExposureTime', '-FNumber', self.source_dir]
            ...
            self.progress.emit(100); self.log.emit("✓ Sorting completed.")
        ...
        if len(group) >= s and len(group) % s == 0:
            for i in range(len(group) // s):
                ...
                if len(set([p['exp'] for p in subset])) > 1:
                    target = os.path.join(base, datetime.fromtimestamp(subset[0]['ts']).strftime('%Y-%m-%d_%H-%M-%S') + f"_{i}_reeks"); os.makedirs(target, exist_ok=True)
...
class HdrWorker(BaseWorker):
    ...
    def run(self):
        try:
            ...
            subdirs = [self.base_dir] if raws_here else sorted([d.path for d in os.scandir(self.base_dir) if d.is_dir() and not d.name.startswith('.')])
            if not subdirs: self.log.emit("No folders found."); self.finished.emit(); return
            xmp = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), CONFIG["DT_XMP_FILE"]); root = os.path.dirname(self.base_dir)
            for i, path in enumerate(subdirs):
                if not self._is_running: break
                name = os.path.basename(path); self.log.emit(f"\n--- Processing: {name} ---"); cfg = os.path.expanduser("~/.cache/panoflow_temp")
                ...
                if self.method in ["hdrmerge", "both"]:
                    ...
                    if res_dng and self.collect:
                        d_dest = os.path.join(root, CONFIG["HDR_COLLECT_NAME"], "DNG"); os.makedirs(d_dest, exist_ok=True); shutil.move(res_dng, os.path.join(d_dest, os.path.basename(res_dng)))
                if self.method in ["enfuse", "both"]:
                    res_tif = self._do_enfuse(path, name, cfg, xmp)
                    if res_tif and self.collect:
                        t_dest = os.path.join(root, CONFIG["HDR_COLLECT_NAME"], "TIFF"); os.makedirs(t_dest, exist_ok=True); shutil.move(res_tif, os.path.join(t_dest, os.path.basename(res_tif)))
    ...
    def _do_enfuse(self, path, name, cfg, xmp):
        ...
        try:
            ...
            if len(tifs) >= 2 and self._is_running:
                ...
                if self._is_running:
                    subprocess.run(['enfuse', '--exposure-weight=1.0', '--saturation-weight=0.5', '--contrast-weight=0.5', '--output', out_h] + sorted(glob.glob(f"{ali}*.tif")), env=ENV_STABLE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("PanoFlow v5.1"); self.setGeometry(100, 100, 800, 650); self.worker = None; self.thread = None; self.s2_manually_set = False
    ...
    def setup_t1(self):
        ...
        h_gap = QHBoxLayout(); h_gap.addWidget(QLabel("Max. tijd tussen reeksen:")); self.gv = QDoubleSpinBox(); self.gv.setRange(0.1, 10.0); self.gv.setValue(1.0); self.gv.setSingleStep(0.1); h_gap.addWidget(self.gv); h_gap.addWidget(QLabel("sec")); h_gap.addStretch(); l.addLayout(h_gap)
        ...
        self.k = QCheckBox("Behoud de eerste foto"); self.k.setChecked(True); l.addWidget(self.k)
        self.b1 = QPushButton("Start Sorteren"); self.b1.clicked.connect(self.go1); l.addWidget(self.b1); self.p1 = QProgressBar(); l.addWidget(self.p1); self.log1 = QTextEdit(); self.log1.setReadOnly(True); l.addWidget(self.log1)
    ...
    def setup_t2(self):
        ...
        self.c1 = QCheckBox("Verzamel resultaten"); self.c1.setChecked(True); l.addWidget(self.c1); self.c2 = QCheckBox("Verwijder reeks-mappen"); self.c2.setChecked(False); l.addWidget(self.c2)
    ...
    def show_inf(self):
        txt = (
            "<h3>PanoFlow (v5.1) - Photo Workflow Automation</h3>"
            "<p>This program automates the management and processing of large quantities of RAW image files, designed for panorama construction.</p>"
            "<b>1. Sorting & Organizing:</b><br>"
            "Groups RAW files into sequences based on capture time (adjustable gap, default 1.0s). "
            "Only the first capture of each sequence and loose photos stay in the source folder. "
            "This creates a clean overview and secures a set of images for stitching a standard panorama if HDR fails.<br><br>"
            "<b>2. Batch HDR Production:</b><br>"
            "Processes sequences into TIFF (via Enfuse) or 32-bit DNG (via HDRmerge). "
            "Processing occurs serially per folder to prevent system overload while utilizing available processor threads. "
            "Orientation fixes are applied automatically.<br><br>"
            "<b>3. Collection & Cleanup:</b><br>"
            "HDR results are moved to 'Verzamelde_HDR_bestanden'. If enabled, temporary RAW folders are deleted after processing.<br><br>"
            "<b>The XMP Profile (oppepper.xmp):</b><br>"
            "Only required for TIFF (Enfuse). Used for <b>lens correction</b> (crucial for alignment) and dynamic range optimization."
        )
        QMessageBox.information(self, "PanoFlow Information", txt)
